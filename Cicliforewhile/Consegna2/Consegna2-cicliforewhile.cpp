...
//Calcola base^potenza. Per esempio 2^3 restituisce 8.
int myPotenza(int base, int esponente) {
	//Questa funzione funziona solo per esponenti positivi
	if(esponente <= 0)
		return 1;

	cout << "Funzione myPotenza(" << base << ", " << esponente << ")" << endl; //Per dare ordine all'output in console

	int risultato = 0;
	for(int i = 1; i <= esponente; i++) { //notare <=. Prova a lanciare il codice per capire! ;)
		risultato = base * base;
		cout << base << "^" << i << " = " << risultato << endl;
	}

	cout << endl; //Per dare ordine all'output in console
